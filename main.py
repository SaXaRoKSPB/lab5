...
language = Label(window, 
                 ...
                 background="midnightblue", foreground="white", font = ("Times New Roman", 14))
...
button = Button(window, text="ГОТОВО", width=15, font = ("Times New Roman", 25), bg = "black", fg = "white", command=clicked)
button.place(relx = 0.12, rely = 0.35)
    
language = word_language
country_name = word_country
city_name = word_city
class WeatherAPI:
    ...
    def __init__(self, city=city_name, country=country_name, lg=language):
        self.city = city
        self.country = country
        self.language = lg

    def requests_place(self):
        parameters = {'q': f'{self.city},{self.country}','limit': 1,'appid': self.token}
    ...
    def requests_weather(self):
        parameters = {'lat': self.lat, 'lon': self.lon, 'units': 'metric', 'appid': self.token, 'lang': self.language}
